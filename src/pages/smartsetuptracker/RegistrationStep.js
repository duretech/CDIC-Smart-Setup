...
useEffect(() => {
  if (selectedSection) {
    ...
    // 1. Unmapped questions (programSectionFlag === false)
    ...
    const availableOptions = attribuetArray
      .filter(attr => {
        ...
        // Include if unmapped
        if (attr.programSectionFlag === false) return true;
        
        // Include if it's in the current section
        if (currentSectionAttributeIds.includes(attrId)) return true;
        
        // Exclude all others (mapped to different sections)
        return false;
      })
      .map(attr => ({
    ...
    // No section selected - show all unmapped questions
    const unmappedOptions = attribuetArray
      .filter(attr => attr.programSectionFlag === false)
      .map(attr => ({
        value: attr.trackedEntityAttributeId || attr.id || attr.name,
        label: attr.name || attr.displayName
      }));
...
const handleSectionChange = (event) => {
  ...
  const pushQuestion = (props) => {
      ...
      arrayHolder.map((el, idx) => {
        if (el.orignalname == values.orignalname) {
          ...
          if (el.type == 'optionset') {
            ...
            el['optionvaluesLocale'] = localeHolder
          }
        }
        else {
          el.isUpdated = false;
    ...
    tempDepentHolder.map(element => {
      if (values.name && _.find(element.dependentdataelementnames, ['childdataelementname', values.name])) {
        if (element.dataelementname != attribuetArray[values.parentQuestion].name) {
          let index = _.findIndex(element.dependentdataelementnames, { 'childdataelementname': values.name })
          element.dependentdataelementnames.splice(index, 1)
        }
      }
    })
    // END
    if (attribuetArray[values.parentQuestion] && _.find(tempDepentHolder, ['dataelementname', attribuetArray[values.parentQuestion].name])) {
      tempDepentHolder.map(el => {
        if (el.dataelementname == attribuetArray[values.parentQuestion].name) {
          if (_.find(el.dependentdataelementnames, ['childdataelementname', values.name])) {


          } else {
            el.dependentdataelementnames.push({ "childdataelementname": values.name })
          }
        }
      })
    } else if (values.parentQuestion) {
      let temp = {}
      temp['dataelementname'] = attribuetArray[values.parentQuestion].name
      temp['matchingvalue'] = values.dependentValue
      temp['dependentdataelementnames'] = [{ "childdataelementname": values.name }]
                ...
                <FForm className="">
                  <Card>
                    ...
                    <Card.Body className="regtabbody">
                                        ...
                                        <InputGroup key={`elm_5` + id} className="mb-3">
                                          ...
                                          <FormControl
                                            ...
                                            onChange={(e) => {
                                              ...
                                              // Update optionValuesLocaleArray without directly modifying nested properties
                                              const updatedLocaleArray = [...optionValuesLocaleArray];
                                              if (updatedLocaleArray[id] && updatedLocaleArray[id][optionsArray[id]]) {
                                                const tempLocale = { ...updatedLocaleArray[id] };
                                                tempLocale[e.target.value] = [...tempLocale[optionsArray[id]]]; // Copy of the existing locale array
                                                updatedLocaleArray[id] = tempLocale;
                                              }
                                              setOptionValuesLocaleArray(updatedLocaleArray);

                                              // Handle questionObject update as before
                            ...
                            return (
                              <>
                                ...
                                <Form.Control as="select" {...field}>
                                  <option value="">-</option>
                                  {attribuetArray.map((element, idx) => {
                                    if (element.type == "optionset") {
                      ...
                      {(formRef.current != null && formRef.current.values.parentQuestion != '' && attribuetArray[formRef.current.values.parentQuestion]) ?
                        <Form.Group>
                            ...
                            {({ field, meta }) => {
                              return (
                                <>
                                  ...
                                  <Form.Control as="select" {...field}>
                                    <option>-</option>
                                    {attribuetArray[formRef.current.values.parentQuestion].optionvalues.map((element, idx) => {
